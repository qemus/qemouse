/*
 * QEMouse - VMware communication routines
...
static void vmware_abspointer_data_clear(void)
{
	...
	/* Drop complete or partial data words already waiting in the queue.
	 * Only the low 16 bits are the queue length; the upper bits are status. */
	while ((data_avail = (uint16_t) ((status = vmware_abspointer_status())
	                                & VMWARE_ABSPOINTER_STATUS_MASK_DATA)) != 0) {
		struct vmware_abspointer_data data;
		unsigned size = data_avail > 4 ? 4 : data_avail;
		vmware_abspointer_data(size, &data);
